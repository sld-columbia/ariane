...
module nfu_wrap import ariane_pkg::*; (
  ...
  localparam int unsigned NR_SHIFT_REG_ENTRIES           = 4;
    ...
    if(~rst_ni) begin 
      ...
      if(nfu_valid_i && fu_data_i.operator == ariane_pkg::SET_LOAD_NFU) begin
        ...
        if(commit_ack_i[0] || commit_ack_i[1]) begin
          ...
          if((commit_ack_i[0] && commit_ack_i[1]) | pending ) begin
  ...
  always_comb begin : decoder
    ...
    // Set valid signals
    if(addr_irf != 0 && (commit_ack_i[0] || commit_ack_i[1]) | pending) begin
      irf_store_valid = 1'b1;
      if(pending) begin
        data = BufferRes;
      end else if(commit_instr_i[0].valid) begin
